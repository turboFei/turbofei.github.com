...
那么调用`internalReleaseLease`， 顾名思义，释放internalLease, 其注释为 `Move a file that is being written to be immutable.`, 也就是说把一个正在被写的文件变为不可改变的状态.
...
- 将文件从正在被写变为不可变状态意味着另外一个dfsClient可以开始对其写入，例如进行append操作。
