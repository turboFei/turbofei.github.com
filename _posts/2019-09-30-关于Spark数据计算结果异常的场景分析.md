...
首先有一个参数控制是否允许dynamicPartitionOverwrite，

在Spark中其参数为`spark.sql.sources.partitionOverwriteMode`(hive中也有对应的参数),默认为`static`，代表不允许进行dynamicPartitionOverwrite，如果设为`dynamic`代表允许dynamicPartitionOverwrite。

针对一个分区表，比如tablea, 其中 p1, p2... pn, n个partition key。

dynamic 的partition overwrite代表，如果在写入操作时，没有完全的指定这些partition key对应的值。
...
而如果你完全指定了所有的partition key的值，即使你允许进行dynamic partition overwrite，依然是进行static的partition overwrite，因为一切都是确定的。
